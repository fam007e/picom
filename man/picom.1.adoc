...
*--backend* _BACKEND_::
	Specify the backend to use: `xrender`, `glx`, or `egl`. `xrender` is the default one.
+
--
...
* `glx` (OpenGL) backend performs all rendering operations with OpenGL. It is more friendly to some VSync methods, and has significantly superior performance on color inversion (*--invert-color-include*) or blur (*--blur-background*). It requires proper OpenGL 3.3 support from your driver and hardware. *--xrender-sync-fence* might be needed on some systems to avoid delay in changes of screen contents.
* `egl` (OpenGL via EGL) backend performs rendering using OpenGL with EGL for context creation instead of GLX. It shares the same rendering capabilities as the `glx` backend and requires OpenGL 3.3 support. EGL is cross-platform and not tied to X11, making it a useful alternative if the `glx` backend exhibits issues. *This backend is still experimental.* On NVIDIA hardware, suspending and resuming with `NVreg_PreserveVideoMemoryAllocations=0` may cause instability; setting that parameter to `1` or using the `glx` backend is recommended in that case.
--
...
The interface between picom and a custom shader is dependent on which backend is being used. The xrender backend doesn't support shader at all. Here we describe the interface provided by the glx and egl backends.
